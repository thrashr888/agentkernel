...
- **Config resolution**: constructor args > `AGENTKERNEL_BASE_URL` / `AGENTKERNEL_API_KEY` env vars > defaults (`http://localhost:8080`, no auth, 30s timeout)
...
cd sdk/nodejs && AGENTKERNEL_URL=http://localhost:8080 npm run test:integration
```
