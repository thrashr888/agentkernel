//! Integration tests for the Kubernetes backend.
//!
//! These tests require a running Kubernetes cluster accessible via kubeconfig.
//! They are `#[ignore]` by default -- run with:
//!
//!   cargo test --test kubernetes_integration_test --features kubernetes -- --ignored --nocapture
...
fn test_orch_config() -> agentkernel::config::OrchestratorConfig {
    agentkernel::config::OrchestratorConfig {
        namespace: "agentkernel-test".to_string(),
        ..Default::default()
    }
}
...
async fn test_kubernetes_lifecycle() {
    use agentkernel::backend::Sandbox;

    let orch_config = test_orch_config();
    let mut sandbox =
        agentkernel::backend::KubernetesSandbox::new("k8s-test-lifecycle", &orch_config);
    ...
    let result = sandbox
        .exec(&["echo", "hello-from-k8s"])
        .await
        .expect("Failed to exec in K8s sandbox");
    assert_eq!(result.exit_code, 0);
    assert!(
        result.stdout.contains("hello-from-k8s"),
        "Expected stdout to contain 'hello-from-k8s', got: {}",
        result.stdout
    );

    // Write a file
    sandbox
        .write_file("/tmp/test.txt", b"kubernetes test content")
        .await
...
#[tokio::test]
#[ignore = "requires Kubernetes cluster"]
async fn test_kubernetes_exec_with_env() {
    use agentkernel::backend::Sandbox;

    let orch_config = test_orch_config();
    let mut sandbox = agentkernel::backend::KubernetesSandbox::new("k8s-test-env", &orch_config);
    let config = test_sandbox_config();
    ...
    let env = vec!["MY_VAR=hello_k8s".to_string()];
    let result = sandbox
    ...
    assert_eq!(result.exit_code, 0);
    assert!(
        result.stdout.contains("hello_k8s"),
...
/// Test network isolation (NetworkPolicy)
#[tokio::test]
#[ignore = "requires Kubernetes cluster with NetworkPolicy support"]
async fn test_kubernetes_network_isolation() {
    use agentkernel::backend::Sandbox;
    ...
    // Try to reach an external endpoint -- should fail with NetworkPolicy
    let result = sandbox
        .exec(&[
            "sh",
            "-c",
            "wget -q -O- http://example.com --timeout=3 2>&1 || true",
        ])
        .await
        .expect("Exec itself should not fail");

    // The command should fail (timeout or connection refused)
...
#[tokio::test]
#[ignore = "requires Kubernetes cluster with sufficient capacity"]
async fn test_kubernetes_concurrent_100() {
    use agentkernel::backend::Sandbox;
    use std::sync::Arc;
    ...
    println!(
        "100 concurrent K8s sandboxes completed in {:.1}s",
        elapsed.as_secs_f64()
    );
    if !errors.is_empty() {
...
#[tokio::test]
#[ignore = "requires Kubernetes cluster"]
async fn test_kubernetes_file_injection() {
