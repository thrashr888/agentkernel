...
| Feature flag | `--features kubernetes` | `--features nomad` |
| Requirements | `kubectl`, cluster access | `nomad` CLI, cluster access |

## Building

Orchestration backends are opt-in via Cargo feature flags:

```bash
cargo build --features kubernetes        # Kubernetes only
cargo build --features nomad             # Nomad only
cargo build --features kubernetes,nomad  # Both
```
...
The CRD YAML can be generated programmatically:
...
For deploying agentkernel as a service on a cluster, see the [Deployment Guide](deploy.md).

- **Kubernetes**: Helm chart at `deploy/helm/agentkernel/`
- **Nomad**: Job definition at `deploy/nomad/agentkernel.nomad.hcl`
