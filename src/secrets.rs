...
struct FileStore {
    /// Map of key -> encrypted value (base64-encoded XOR with machine-id for basic obfuscation)
...
impl SecretVault {
    ...
    fn file_set(&self, key: &str, value: &str) -> Result<()> {
        ...
        store
            .secrets
            .insert(key.to_string(), Self::obfuscate(value));
    ...
    fn file_get(&self, key: &str) -> Result<Option<String>> {
        ...
        match store.secrets.get(key) {
            Some(encoded) => Ok(Some(Self::deobfuscate(encoded)?)),
            None => Ok(None),
        }
    }
